...
class ChefOrchestrateur:
    ...
    @agent_tool
    async def expert_finance(self, question: str) -> str:
        """Consulte l'expert en finance pour les questions de budget, emprunt, coûts, fiscalité"""
        return await self.route_to_expert(question, 'finance')
    
    @agent_tool
    async def expert_guide_acquereur(self, question: str) -> str:
        """Consulte l'expert guide acquéreur pour les questions d'acquisition, étapes, promoteurs"""
        return await self.route_to_expert(question, 'guide_acquereur')
    
    @agent_tool
    async def expert_materiaux_prestataire(self, question: str) -> str:
        """Consulte l'expert matériaux/prestataires pour les questions de matériaux, fournisseurs, construction"""
        return await self.route_to_expert(question, 'materiaux_prestataire')
...
def create_chef_orchestrateur():
    ...
    agent = LlmAgent(
        ...
        tools=[
            chef_instance.expert_finance,
            chef_instance.expert_guide_acquereur,
            chef_instance.expert_materiaux_prestataire
        ]
    )
